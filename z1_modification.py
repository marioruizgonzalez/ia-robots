...
def play_movements_and_sound():
    ...
    if movimientos_guardados:
        for movimiento in movimientos_guardados:
            ...
            posiciones_actuales = [scale.get() for scale in scales]

            # Calcular los pasos necesarios para cada servo
            steps = 10  # Número de pasos para la interpolación
            step_sizes = [(movimiento[i] - posiciones_actuales[i]) / steps for i in range(len(movimiento))]
            ...
            for step in range(steps):
                ...
                # Calcular nuevas posiciones para cada servo
                nuevas_posiciones = [posiciones_actuales[i] + step_sizes[i] * (step + 1) for i in range(len(movimiento))]

                # Mover los servos a las nuevas posiciones
                for i, new_position in enumerate(nuevas_posiciones):
                    scales[i].set(new_position)
                    move_servo(i + 1, new_position)

                # Pequeña pausa para suavizar el movimiento
                time.sleep(velocidad_var.get() / 1000.0 / steps)
